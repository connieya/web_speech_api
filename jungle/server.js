...
wsServer.on("connection", (socket) => {
  ...
  // 방에 들어옴
  socket.on("join_room", (roomName, time) => {
  ...
  socket.on("send_message", (msgData) => {
    console.log(`서버에서 메세지 받아짐?? ${msgData}`);
    // if (msg["message"] === "기록시작@") {
    //   room_to_scribe[room] = true;
    //   console.log("기록시작");
    //   socket.to(room).emit("scribe_start", "기록시작");
    //   room_to_scribe_restart[room].push(
    ...
    //   }
    //   msg["talking_begin_time"] =
    //     msg["talking_begin_time"] - room_to_start[room];
    ...
    //   socket
    //     .to(room)
    //     .emit("new_message", `${socket.nickname}: ${msg["message"]}`);
    //   // 인자로 받은 함수FE에서 실행
    //   done();
    // }
  });
  ...
  socket.on("disconnecting", () => {
    ...
    if (countRoom(socket["roomName"]) === 1) {
      ...
      console.log(
        socket["roomName"],
        "번방 회의 진행시간 :",
        end_time - room_to_start[socket["roomName"]]
      );
